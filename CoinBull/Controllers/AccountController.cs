                ...
                if (result.Succeeded)
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        using (var command = new SqlCommand("AddJob", connection))
                        {
                            ...
                            command.Parameters.Add("@user", SqlDbType.VarChar).Value = model.Email;
                            command.Parameters.Add("@coins", SqlDbType.VarChar).Value = model.Coins;
                            command.Parameters.Add("@percent", SqlDbType.Int).Value = Convert.ToInt32(model.Percent);
                            command.Parameters.Add("@minutes", SqlDbType.Int).Value = Convert.ToInt32(model.Minutes);

                            await command.ExecuteNonQueryAsync();

                        }

                        //add all the coins to notification list
                        foreach(string c in coinsList.Split(','))
                        {
                            using (var command = new SqlCommand("AddNotificationsList", connection))
                            {

                                command.CommandType = CommandType.StoredProcedure;

                                command.Parameters.Add("@user", SqlDbType.VarChar).Value = model.Email;
                                command.Parameters.Add("@coin", SqlDbType.VarChar).Value = c.Trim();
